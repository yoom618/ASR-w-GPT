...
class Mamba(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList([
            nn.ModuleList(
                [
                    MambaBlock(d_input=d_input, d_output=d_input, **mamba_par) if i != num_layers - 1 \
                        else MambaBlock(d_input=d_input, d_output=d_output, **mamba_par),
                    RMSNorm(d_input)
                ]
            )
        ...
        if residual:
            if d_input != d_output:
                self.residual = nn.Linear(d_input, d_output)
            else:
    ...
    def forward(self, seq : Tensor, cache : Cache = None) -> Tuple[Tensor, Cache]:
        ...
        Args:
            seq (Tensor): Input sequence of shape (batch_size, seq_len, d_input).
            
        Returns:
            Tensor: Output sequence of shape (batch_size, seq_len, d_output).
        '''
...
class MambaBlock(nn.Module):
    ...
    def __init__(
        ...
        self.out_proj = nn.Linear(d_model, d_output, bias=False)
