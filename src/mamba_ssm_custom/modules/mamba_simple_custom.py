...
try:
    from mamba_ssm_custom.ops.triton.selective_state_update import selective_state_update
except ImportError:
    selective_state_update = None

try:
    from mamba_ssm_custom.ops.triton.layer_norm import RMSNorm, layer_norm_fn, rms_norm_fn
except ImportError:
...
class GatedMamba(nn.Module):
    def __init__(
        ...
        if timevariant_dt | timevariant_B | timevariant_C:
            if timevariant_dt:
                self.output_dim[0] = self.dt_rank
            if timevariant_B:
                self.output_dim[1] = self.d_state
            if timevariant_C:
                self.output_dim[2] = self.d_state
        self.x_proj = nn.Linear(
            self.d_inner, sum(self.output_dim), bias=False, **factory_kwargs
        )
        if not timevariant_B:
            self.B = nn.Parameter(torch.zeros(self.d_inner, self.d_state, **factory_kwargs))
            self.B._no_weight_decay = True
        if not timevariant_C:
            self.C = nn.Parameter(torch.zeros(self.d_inner, self.d_state, **factory_kwargs))
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        """
        batch, seqlen, diminput = hidden_states.shape
        ...
        A = -torch.exp(self.A_log.float())  # always have negative values. (d_inner, d_state)
        ...
        if self.use_fast_path and causal_conv1d_fn is not None and inference_params is None:  # Doesn't support outputting the states
            ...
            dt, B, C = torch.split(x_dbl, self.output_dim, dim=-1)  # (batch * seqlen, dt_rank), (batch * seqlen, d_state), (batch * seqlen, d_state) if enabled
            if not self.tv_dt:
                dt = self.dt_proj.bias.float()  # (d_inner,)
                dt = dt.view(1, -1, 1).expand(batch, self.dt_rank, seqlen)  # (batch, d_inner, seqlen)
            else:
                dt = self.dt_proj.weight @ dt.t()  # self.dt_proj.weight : (d_inner, d_rank), dt : (d_rank, batch * seqlen) -> (d_inner, batch * seqlen)
            ...
            y = selective_scan_fn(
                ...
                self.D.float(),
                z=z,
                delta_bias=self.dt_proj.bias.float() if self.tv_dt else None,
                delta_softplus=True,
    ...
    def step(self, hidden_states, conv_state, ssm_state):
        ...
        dt, B, C = torch.split(x_db, [self.dt_rank, self.d_state, self.d_state], dim=-1)  # (B, dt_rank), (B, d_state), (B, d_state)
