...
class Wav2Vec2GPTModel(Wav2Vec2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.rnn_compressor = nn.RNN(
            input_size=config.output_hidden_size, 
#             input_size=config.hidden_size, 
            
#             bidirectional=False, hidden_size=1, 
#             bidirectional=False, hidden_size=(config.hidden_size + 1), 
            bidirectional=False, hidden_size=config.hidden_size, 
#             bidirectional=True, hidden_size=int(config.hidden_size), 
            num_layers=1, 
            batch_first=True,
        )
...
#             attention_mask=input_attention_mask,
        ...
        hidden_states, _ = self.rnn_compressor(hidden_states) # size: (batch_size, seq_len, config.hidden_states + 1)
...
#         peak = - cos(hidden_states[...,:-1,:], hidden_states[...,1:,:])
        peak = nn.functional.softmax(self.lm_head(hidden_states), dim=-1)
        peak = (peak[...,1:,:] - peak[...,:-1,:]).max(dim=-1)[0]
        
        
#         peak_values, peak_indices = self.adaPool(peak)
        peak_min, threshold, peak_max = -1.0, 0.2, 1.0
        rand_arr = threshold - torch.rand_like(peak) * (threshold - peak_min)
        _, peak_indices = self.adaPool(torch.where(peak > threshold, peak, rand_arr))
...
#             loss_ctc = loss_fct_0(lm_logits_0.contiguous().view(-1, lm_logits_0.size(-1)), labels.contiguous().view(-1))
...
#             log_probs = nn.functional.log_softmax(lm_logits_0, dim=-1, dtype=torch.float32).transpose(0, 1)
