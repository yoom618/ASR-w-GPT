...
class Wav2Vec2AdapterLayer(nn.Module): ############# MODIFIED #############
    def __init__(self, layer_idx, config):
        ...
        input_size = config.conv_dim[-1] if (layer_idx == 0) else config.output_hidden_size
#         output_size = 2 * (config.hidden_size if (config.num_adapter_layers - layer_idx == 1) else config.output_hidden_size)
        output_size = 2 * config.output_hidden_size
...
class Wav2Vec2ForPreTraining(Wav2Vec2PreTrainedModel):
    ...
    def forward(
        ...
        >>> with torch.no_grad():
        transformers.     outputs = model(input_values, mask_time_indices=mask_time_indices)
...
class Wav2Vec2ForMaskedLM(Wav2Vec2PreTrainedModel):
    ...
    def forward(
        ...
        transformers.     speech, _ = sf.read(batch["file"])
        transformers.     batch["speech"] = speech
        transformers.     return batch
    ...
    def __init__(self, config):
        ...
        if config.vocab_size is None:
            raise ValueError(
                ...
                "instantiate the model as follows: `Wav2Vec2ForCTC.from_pretrained(transformers., vocab_size=vocab_size)`. "
